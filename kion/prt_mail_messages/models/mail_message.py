...
from odoo.tools import html2plaintext, html_escape, html_to_inner_content, misc
...
class MailMessage(models.Model):
    ...
    plain_body = fields.Text(compute="_compute_plain_body", store=True)
    ...
    state = fields.Selection(
        [('selected', 'selected'),('cancel','Canceled'),('reading','Reading')], string='state', default='reading')


    def canceled(self):
        try:
            self.state = 'cancel'
            print(self.author_allowed_id.name)
            print(self.env.context.get('active_id'))
            active_id = self.env.context.get('active_id')
            active_model = self._context.get('active_model')

            x = self.env[active_model].search([('id', '=', active_id)])

            x.relationallll =''
        except:
            raise UserError(_("you should cancel email inside related form"))
    ...
    def Selected(self):
        try:
            ...
            x.relationallll=self.id
            x.state = 'select_offer'
            print("attttttachh")
            # print(self.attachment_ids.read())
            ...
            raise UserError(_("you should select email inside related form"))
    ...
    @api.depends("body")
    def _compute_plain_body(self):
        for rec in self:
            rec.plain_body = html2plaintext(rec.body)
    ...
    def unlink_rights_check(self):
        ...
        if self.state == 'selected':
            raise AccessError(_("You cannot delete selected messages!"))
    ...
    def _get_message_preview(self, max_char=DEFAULT_MESSAGE_PREVIEW_LENGTH):
        ...
        return super(MailMessage, self)._get_message_preview(max_char=max_preview)
    ...
    def _compute_subject_display(self):
        ...
        for rec in self.with_context(bin_size=False):
            ...
            rec.subject_display = TREE_TEMPLATE % {
                ...
                "record_ref": "{}: {}".format(
                    rec.model_name, rec.record_ref.display_name
                )
                if rec.record_ref
                else "",
                "icons": notification_icons,
                "display_number_days_after_deletion": rec._display_number_days_after_deletion()
                or "",
                "body": rec.preview,
            }
    ...
    def reply_prep_context(self):
        ...
        if wizard_mode in ["quote", "forward"]:
            body = BLOCK_QUOTE.format(
                misc.format_datetime(self.env, self.date),
                self.author_display,
                self.subject or "",
                self.body or "",
            )
        return {
            ...
            "default_parent_id": False if is_forward else self.id,
            "default_model": self.model,
            "default_partner_ids": self.author_allowed_id.ids,
            "default_attachment_ids": self.attachment_ids.ids if is_forward else [],
