...
class ClaimsModel(models.Model):
    _name = 'after.fix'
    _description='after fix model '


    name=fields.Char(string='الساده:شركة ')
    topic1=fields.Char(string='اسم المالك ')
    topic2=fields.Char(string='رقم وثيقة التامين العميل ')
    topic3=fields.Char(string='نوع السياره ')
    topic4=fields.Char(string='موديل السياره ')
    topic5=fields.Char(string='سنة الصنع ')
    topic6=fields.Char(string='رقم الشاسية ')
    topic7=fields.Char(string='رقم اللوحة ')
        ...
        for rec in self:
            ...
            if all_eamils:
                ...
                for emails in all_eamils:
                    mail_template = self.env.ref('operation.mail_after_fix_template1dd1')
