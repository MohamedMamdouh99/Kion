...
class PayrollReport(models.AbstractModel):
    ...
    def generate_xlsx_report(self, workbook, data, lines):
        ...
        for used_struct in used_structures:
            ...
            sheet.write(2, 3, 'Cost Center', format1)
            sheet.write(2, 4, 'Work Location', format1)
            sheet.write(2, 5, 'Position', format1)
            sheet.write(2, 6, 'تاريخ بداية العقد ', format1)
            for rule in rules:
            ...
            for slip in lines.slip_ids:
                if lines.slip_ids:
                    if slip.struct_id.id == used_struct[0]:
                        date_start_date = str(slip.employee_id.first_contract_date.strftime('%B %d, %Y'))
                        ...
                        sheet.write(e_name, 3, slip.employee_id.cost_center, format3)
                        sheet.write(e_name, 4, slip.employee_id.work_location_id.name, format3)
                        sheet.write(e_name, 5, slip.employee_id.job_title, format3)
                        sheet.write(e_name, 6, date_start_date, format3)
                        ...
                        bank_account_value = slip.employee_id.bank_account
                        sheet.write(x, col_no, bank_account_value, format3)
                        sheet.write(2, col_no, "Bank Account", format1)
